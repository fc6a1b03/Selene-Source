...
class AdminSourceManagementPanel extends StatefulWidget {
  const AdminSourceManagementPanel({
    super.key,
  ...
  @override
  State<AdminSourceManagementPanel> createState() =>
      _AdminSourceManagementPanelState();
}

class _AdminSourceManagementPanelState
    extends State<AdminSourceManagementPanel> {
  ...
  @override
  void didUpdateWidget(covariant AdminSourceManagementPanel oldWidget) {
        ...
        return Column(
          ...
          children: <Widget>[
            AdminSectionHeader(
              ...
              title: '视频源',
              description: '支持新增、启用、禁用、删除、有效性检查和排序保存。窄屏自动换行，宽屏自动扩列。',
  ...
  List<Widget> _buildHeaderActions() {
    return <Widget>[
      FilledButton.icon(
        ...
        icon: Icon(
          _isValidating ? LucideIcons.loaderCircle : LucideIcons.badgeCheck,
          size: 16,
        ),
        label: Text(_isValidating ? '检测中...' : '有效性检查'),
      ),
      FilledButton.tonal(
        onPressed: !_orderDirty || _loadingKeys.contains('save_order')
            ? null
            : _handleSaveOrder,
        child: Text(_loadingKeys.contains('save_order') ? '保存中...' : '保存排序'),
      ),
      if (_selectedSourceKeys.isNotEmpty)
        FilledButton.tonal(
      ...
      if (_selectedSourceKeys.isNotEmpty)
        FilledButton.tonal(
          onPressed:
              _loadingKeys.contains('batch_delete') ? null : _handleBatchDelete,
          child: Text(
      ...
      FilledButton.tonal(
        ...
        child: Text(
          _selectedSourceKeys.length == _sources.length && _sources.isNotEmpty
              ? '清空选择'
              : '全选',
        ),
      ),
      ...
      child: Column(
        children: <Widget>[
          ...
          Align(
            ...
            child: FilledButton(
              onPressed:
                  _loadingKeys.contains('add_source') ? null : _handleAddSource,
              child: Text(
  ...
  Widget _buildValidationCard() {
    final List<AdminSourceValidationItem> items =
        _validationResults.values.toList(growable: false)
          ..sort((AdminSourceValidationItem a, AdminSourceValidationItem b) {
            return a.name.compareTo(b.name);
          });
    final List<String> validKeys = items
        .where(
          (AdminSourceValidationItem item) =>
              item.status == AdminSourceValidationStatus.valid,
        )
        .map((AdminSourceValidationItem item) => item.key)
        ...
        children: <Widget>[
          ...
          ConstrainedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (BuildContext context, int index) {
                final AdminSourceValidationItem item = items[index];
                return Row(
                  children: <Widget>[
                    Expanded(
                      child: Text(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _buildSelectableLine('接口', source.api, maxLines: 2),
  ...
  Future<void> _handleSaveOrder() async {
    final bool success = await _runAction(
      ...
      action: () => widget.onAction(
        action: 'sort',
        order: _sources.map((AdminSourceConfig item) => item.key).toList(
              growable: false,
            ),
      ),
    );
  ...
  void _handleToggleSelectAllSources() {
    setState(() {
      if (_selectedSourceKeys.length == _sources.length &&
        ...
        _selectedSourceKeys
          ..clear()
          ..addAll(_sources.map((AdminSourceConfig item) => item.key));
      }
    });
  }
  ...
  void _syncSources() {
    ...
    _selectedSourceKeys.removeWhere(
      (String key) =>
          !_sources.any((AdminSourceConfig item) => item.key == key),
    );
    _orderDirty = false;
  }
}
